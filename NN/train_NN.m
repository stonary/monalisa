...
function [train_CE_list, valid_CE_list, W1, b1, W2, b2] = train_NN(train_inputs, ... 
   ...
   %% initialize
    ...
    for epoch = 1:num_epochs
      ...
      train_CE = -mean(mean(train_labels .* log(prediction') + (1 - train_labels) .* log(1 - prediction')));
      train_classErr = 1 - sum(train_labels == (prediction'>= 0.5)) / double(num_train_cases);
      % Compute deriv
      ...
      prediction = 1 ./ (1 + exp(-logit));  % Output prediction.
      valid_CE = -mean(mean(valid_labels .* log(prediction') + (1 - valid_labels) .* log(1 - prediction')));
      valid_classErr = 1 - sum(valid_labels == (prediction' >= 0.5)) / double(num_valid_cases);
      ...
      if (mod(total_epochs, 100) == 0 || total_epochs < 11)
